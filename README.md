...
- 普通文稿、画面和播放进度变化先标记为待保存，默认最多每 5 秒合并写入一次；创建/销毁房间、暂停或跳转、权限变更、成员移除以及服务关闭会立即保存。
- 每次成功保存前会把上一次快照保留为 `data/rooms.json.bak`；启动时会清理中断写入留下的临时文件，主快照损坏时自动尝试备份。
...
- 全文编辑器支持导入、导出以及最近 10 份本机稿件备份。
- 停止输入 30 秒后保存一份去重备份；清空、重置、导入和恢复前也会先备份当前稿件。
- 每次文稿修改必须携带当前 `scriptRevision`。版本过期或缺失时，服务端拒绝覆盖并返回最新快照；被拒绝的文稿自动保存为“同步冲突”本机备份。
- v3 同步协议和房间快照不兼容旧版本；首次启动发现旧快照时，会自动将其改名为 `data/rooms.json.legacy-时间戳.bak` 后以空数据启动。浏览器使用全新的 v3 本地存储键，不会恢复旧房间身份。
