...
- **显示端**：只显示提词画面，隐藏设置、编辑和播放控件，并禁止本地调整。
...
测试会启动隔离的临时 Node.js 服务并模拟多台 WebSocket 设备，覆盖三种房间模式、四种设备用途、权限变更、状态与播放同步、房主断线重连与移交、主动关房、空房销毁、服务重启、异常消息、超大消息及同一设备多开页面接管。浏览器端还应回归桌面/手机布局、二维码邀请自动加入、低速滚动和断线自动恢复。
